...
def get_dataset(tokenizer, train_file, dev_file, test_file, train_batch_size, dev_batch_size, test_batch_size, embedding_dim):
    ...
    """ 
    print('Tokenizing train tweets')
    for i in tqdm(range(100)):    
        train_tokens = [tokenizer(s) for s in train_tweets]
    print('Converting train embeddings')
    train_embeddings = global_vectors(train_tweets, lower_case_backup=True)

    sys.exit(1)
	
    print('Converting dev embeddings')
    for i in tqdm(range(100)):    
        dev_embeddings   = [global_vectors.get_vecs_by_tokens(tokenizer(s), lower_case_backup=True) for s in dev_tweets]
    
    print('Converting test embeddings')
    for i in tqdm(range(100)):    
        test_embeddings  = [global_vectors.get_vecs_by_tokens(tokenizer(s), lower_case_backup=True) for s in test_tweets]
    """
